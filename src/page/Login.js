...
const Login = () => {
  ...
  const login = async () => {
    ...
    try {
      ...
      const { data } = await autoFetch.post("v1/singin", {
        email,
        password,
        rememberPassword,
      });
      setNameAndToken(data.data, data.data["token"]);
        ...
        <div className="w-full md:w-[80%] mx-auto flex items-center justify-center  md:justify-between z-[1] md:mt-4 ">
          <div className="bg-[#FEE7D6] dark:bg-[#3a3a3a] dark:text-white/70 w-[90%] md:w-auto px-[20px] md:px-[80px] py-[30px] md:py-[40px] rounded-3xl ">
            <div className=" mb-[18px] ">
              <img
                src={`/images/${dark ? "login-dark.png" : "login.png"}`}
                alt="login"
          ...
          <img
            src={`/images/${dark ? "chicken.png" : "chicken-dark.png"}`}
            alt="chicken"
