...
const Comment = ({
  ...
  const likeComment = async () => {
    ...
    try {
      const { data } = await autoFetch.put(`/api/post/likes-comment`, {
        postId,
        commentId: comment._id,
      });
      setComment({ ...comment, likes: data.comment.likes });
    } catch (error) {
      console.log(error);
    }
  ...
  const handleAddReply = async () => {
    try {
      ...
      try {
        ...
        const { data } = await autoFetch.put("v1/reply_commnent", {
          blog_id: postId,
          comment_id: comment._id,
          text: textReply,
          author_id: userId,
        });
        setComment({ ...comment, replies: data.data.replies });
  ...
  const handleDeleteReplyComment = async (replyId) => {
    ...
    try {
      const { data } = await autoFetch.put(`/api/post/delete-reply-comment`, {
        postId,
  ...
  if (editComment) {
    return (
      <div className="flex gap-x-1.5 py-1 ">
        <img
          src={comment.author?.profilePictrue}
          alt="user_avatar"
      ...
      <div>
        ...
        {comment.replies.length > 0 && (
          ...
          >
            ...
            {showReply
              ? "Hide replies "
              : `View more ${comment.replies.length}
        ...
        <>
          ...
          <div className="flex gap-x-1.5 px-[10%] pt-2 w-full relative items-center ">
            ...
            >
              ...
              <button type="submit" disabled={replyLoading || !textReply}>
                {replyLoading ? (
                  <ReactLoading
                    type="spin"
                    width={20}
