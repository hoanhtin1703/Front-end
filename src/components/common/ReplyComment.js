...
const ReplyComment = ({
  ...
  console.log("replyComment", currentComment);
  const unlikeReplyComment = async () => {
    setLikeReplyLoading(true);
    try {
      const { data } = await autoFetch.put(`/api/post/unlike-reply-comment`, {
        postId,
        commentId,
  ...
  const likeReplyComment = async () => {
    ...
    try {
      ...
      setReplyComment({ ...replyComment, likes: data.reply.like });
    } catch (error) {
      console.log(error);
    }
    setLikeReplyLoading(false);
  ...
  if (editComment) {
    return (
      <div className="flex gap-x-1.5 py-1 ">
        <img
          src={replyComment.author.profilePicture}
          alt="user_avatar"
          ...
          >
            ...
            <button
              ...
              disabled={
                editLoading ||
                !text ||
                (text === replyComment.text && !replyComment.image)
              }
            >
              {editLoading ? (
                <ReactLoading
  ...
  return (
    <div className={`flex gap-x-1.5 mt-1.5 group pl-6 `}>
      ...
      <img
        ...
        onClick={() => {
          navigate(`/profile/${replyComment.author._id}`);
        }}
      />
        ...
        <div className="flex items-center w-full gap-x-1 ">
          <div className="rounded-xl bg-[#F0F2F5] dark:bg-[#3A3B3C] px-3 py-2 max-w-full relative  ">
            ...
            >
              ...
              {replyComment.author.isAdmin === true && (
            ...
            {userId === replyComment.author._id && (
              <div
              ...
              >
                ...
                >
                  <li
                    ...
                    onClick={() => {
                      toast("This function is uploading...");
                    }}
                  >
                    Edit
                  </li>
                  <li
                    ...
                    onClick={() => handleDeleteReplyComment(replyComment._id)}
                  >
                    Delete
                  </li>
                </ul>
            ...
            {replyComment.likes && replyComment.likes.length > 0 && (
              <div
                className={`absolute bottom-2 bg-inherit p-1 ${
                  replyComment.likes && replyComment.likes.length > 1
                    ? "px-2 right-[-30px]"
                    : "right-[-20px]"
              ...
              >
                ...
                {replyComment.likes.length > 1 ? replyComment.likes.length : ""}
              </div>
            )}
          </div>
        </div>
