    ...
    return (
      ...
      >
        ...
        <div className={`${!loading && "hidden"}`}>
          <ReactLoading type="bubbles" width={25} height={25} color="#6A7583" />
        </div>
      </div>
    );
  };
        ...
        >
          {images.length > 0 ? (
            images.map((i, k) => (
              ...
              >
                <img
                  src={`http://localhost:7000/img/${i.image_url}`}
                  alt="image"
                  className={`w-full h-full absolute top-0 left-0 object-cover ${
                    rounded.includes(k)
                      ? `rounded-${positionRounded[rounded.indexOf(k)]}-lg`
                      : ""
                  } `}
                />
              </div>
            ...
            <div className="text-center my-3 col-span-3 ">No image found!</div>
          )}
        </div>
      </div>
    );
